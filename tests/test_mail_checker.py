...
class TestMailChecker(unittest.TestCase):
    ...
    def test_check_mail_success(self, mock_email_from_bytes):
        ...
        mock_part.get_payload.return_value = (
            b"Click here: https://example.com/account/update-primary-location?token=123"
        )
        ...
        self.mail_checker.mail.fetch.return_value = (
            "OK",
            [(b"1", (b"RFC822", b"email content"))],
        )

        link = self.mail_checker.check_mail()
        self.assertEqual(
            link, "https://example.com/account/update-primary-location?token=123"
        )
    ...
    def test_check_mail_non_multipart(self, mock_email_from_bytes):
        ...
        mock_msg.get_payload.return_value = (
            b"Click here: https://example.com/account/update-primary-location?token=456"
        )
        mock_email_from_bytes.return_value = mock_msg
        ...
        link = self.mail_checker.check_mail()
        self.assertEqual(
            link, "https://example.com/account/update-primary-location?token=456"
        )
    ...
    def test_check_mail_no_email(self):
        self.mail_checker.mail.select.return_value = ("OK", [b"1"])
        self.mail_checker.mail.search.return_value = (
            "OK",
            [b""],
        )  # Empty list of mail IDs
        result = self.mail_checker.check_mail()
    ...
    def test_click_button_fail(self):
        self.mail_checker.driver.find_element.side_effect = Exception(
            "Element not found"
        )
        result = self.mail_checker.click_button("https://example.com")
    ...
    def test_init_webdriver_failure(self):
        with patch(
            "mail_checker.webdriver.Chrome", side_effect=Exception("WebDriver Error")
        ):
            checker = MailChecker()
    ...
    def test_init_mailbox_failure(self):
        with patch(
            "mail_checker.imaplib.IMAP4_SSL", side_effect=Exception("IMAP Error")
        ):
            checker = MailChecker()
